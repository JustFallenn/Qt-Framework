...
void DbManager::createDbConnection()
{
    ...
    db.setDatabaseName("/home/stajer/Desktop/project.db");
    db.open();
...
std::vector<QString> DbManager::dataCollection() {
    ...
    if (qry.exec(qryStr)) {
        ...
        while (qry.next()) {
            QString track = QString::number(qry.value(0).toInt()) + ";" + QString::number(qry.value(1).toInt()) + ";" + QString::number(qry.value(2).toInt());
            qDebug() << "Track: " << track;
